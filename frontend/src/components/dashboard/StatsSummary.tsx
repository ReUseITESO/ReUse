    ...
    <div className="grid gap-4 sm:grid-cols-3">
      {STATS_CONFIG.map(stat => {
        ...
        return (
          <div
            key={stat.key}
            className={`rounded-xl border border-gray-200 border-l-4 ${stat.borderColor} bg-white p-5 shadow-sm`}
          >
      ...
      <div className="rounded-xl border border-gray-200 border-l-4 border-amber-400 bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between">
        ...
        <p className="mt-1 text-sm text-gray-500">Puntos</p>
      </div>
    </div>
  );
}
