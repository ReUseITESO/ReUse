...
export default function ZooomSlider(props: ZoomSliderProps) {
	const { zoom, onChange, min = .5, max = 6, step = 0.25 } = props;
	return (
		<div className="slidecontainer flex items-center justify-center gap-4 bg-primary/5 p-4 rounded">
			<button onClick={() => onChange(Math.max(zoom - step, min))} className="px-2 py-1 bg-primary/20 rounded-l">-</button>
			...
			<button onClick={() => onChange(Math.min(zoom + step, max))} className="px-2 py-1 bg-primary/20 rounded-r">+</button>
