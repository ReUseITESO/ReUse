...
class UserStoryGenerator:
    ...
    def generate(
        ...
        duplicates = self.duplication_checker.check_duplicates(title, existing_stories)
...
def main():
    ...
    parser.add_argument(
        "--github", action="store_true", help="Create GitHub Issue from story"
    )
    ...
    def maybe_create_github_issue(story):
        ...
        if args.github:
            ...
            success, message = create_issue_from_story(
                story=story, labels=labels, project=args.project, assignee=args.assignee
            )
            print(message)
            return success
        return True
    ...
    if args.interactive:
        ...
        if create_issue == "y":
            ...
            if not ready:
                ...
                assignee = (
                    input("Assignee GitHub username (press Enter to skip): ").strip()
                    or None
                )
                labels_input = input(
                    "Labels comma-separated (default: user-story): "
                ).strip()
                labels = labels_input.split(",") if labels_input else ["user-story"]

                success, message = create_issue_from_story(
                    story=story, labels=labels, project=project, assignee=assignee
                )
                print(message)
    ...
    if args.input:
        ...
        if args.existing_stories:
            with open(args.existing_stories, "r") as f:
                ...
                existing_stories = [ExistingStory(**s) for s in stories_data]
