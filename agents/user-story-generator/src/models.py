...
class UserStory:
    ...
    def validate(self) -> tuple[bool, List[str]]:
        ...
        for criterion in self.acceptance_criteria:
            if any(word in criterion.lower() for word in ["given", "when", "then"]):
                errors.append(
                    f"Acceptance criteria should not use Given/When/Then format: {criterion}"
                )

        if not self.implementation.backend and not self.implementation.frontend:
            errors.append(
                "Implementation details are missing for both backend and frontend"
            )
    ...
    def is_too_large(self) -> tuple[bool, Optional[str]]:
        ...
        if total_items > 15:
            return (
                True,
                "This story has too many implementation items. Consider splitting into smaller stories.",
            )

        if len(self.acceptance_criteria) > 8:
            return (
                True,
                "This story has too many acceptance criteria. Consider splitting by feature area.",
            )
