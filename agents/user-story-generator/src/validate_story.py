...
class StoryFormatValidator:
    ...
    REQUIRED_SUBSECTIONS = ["### Backend", "### Frontend", "### Database"]
    ...
    def _check_acceptance_criteria(self):
        ...
        if criteria_count < 2:
            self.warnings.append(
                f"Only {criteria_count} acceptance criteria found. Consider adding more."
            )

        if criteria_count > 10:
            self.warnings.append(
                f"{criteria_count} acceptance criteria found. Story might be too large."
            )
    ...
    def _check_implementation_details(self):
        ...
        if not has_backend and not has_frontend:
            self.errors.append(
                "Implementation details must include Backend or Frontend subsection"
            )

        if not has_database:
            self.warnings.append(
                "No Database subsection found. Consider if database changes are needed."
            )
